 ...
 * @param callback
 */
module.exports.find = function (extension, criteria, limit, skip, callback) {
    var collection = applicationStorage.mongo.collection(extension + "_dungeons");
    collection.find(criteria, {_id: 0}).sort({level: -1, time: 1}).limit(limit).skip(skip).toArray(function (error, runs) {
        callback(error, runs);
    });
};
